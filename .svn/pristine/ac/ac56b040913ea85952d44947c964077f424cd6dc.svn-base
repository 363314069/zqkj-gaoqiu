    ...
    for(var j=0;j<jsonObj.length;j++){
        ...
        for(var i=0;i<timeArr.length-1;i++){
            ...
            if(timeArr[i].substring(0,2) > 12){//设置上下午
                if(!compareDate(formatDateTime(curTime),timeArr[i],new Date(date))){//判断当前时间是否大于时间段
                ...
                if(!compareDate(formatDateTime(curTime),timeArr[i],new Date(date))){
                    if(teaArr.indexOf(timeArr[i]) > -1){//该时间段是否在关闭的数组中
...
function timeItemOn(price,time,sum,thisA) {
    ...
    countPrice();//重新计算 选择的数量*选择时间段的金额
}
...
//加载关闭时间段的数据
function closeCurTeatime(date) {
	var curTime = new Date();
	curTime = curTime.setHours(curTime.getHours() + 4);
	curTime = new Date(curTime);
	curTime = formatDateYmdhms(curTime);
	var a = $("#morningTime").children("a");
	var b = true;
    a.each(function(i, obj){
		var time = $(this).children().first().text();
		$(this).attr('id', 'time' + time.replace(":","s"));
		if(date + " " + time < curTime){
			$(this).addClass('closetime')
		} else {
			if(b){
				$("#paramTime").text(time);
				b = false;
...
//时间格式化
var formatDateYmdhms = function (date) {
    var y = date.getFullYear();
    var m = date.getMonth() + 1;
    m = m < 10 ? ('0' + m) : m;
    var d = date.getDate();
