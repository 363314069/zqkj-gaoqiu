...
var footer = {
	...
	style: $("<style>"),
	init:function (){
		this.style.append('.footer-body-height{padding-bottom: .88rem;}')
		this.style.append('.footer{z-index: 9999; display: -webkit-flex; text-align: center; display: flex; font-size:.28rem; justify-content: space-around; background:#fff; width: 100%; height: .88rem; border-top: 0.01rem solid #ccc; position: fixed; left: 0; bottom: 0; height: 0.88rem; }');
		this.style.append('.footer li:nth-child(3){display: none;}');
		this.style.append('.footer .iconfont{font-size: .5rem; display: block; line-height: 0.6rem;}');
		this.style.append('.footer a{font-size: .24rem; display: block; line-height: 0.28rem;}');
		this.style.append('.footer .select .iconfont,.footer .select a{color: #00aa65;}');
		var li1 = $("<li>");
	...
	load:function(){
		if(this.initState){
			if(!this.state){
				$("body").removeClass("footer-body-height");
				this.dom.hide();
			} else {
				$("body").append(this.style).append(this.dom);
				$("body").addClass("footer-body-height");
				this.dom.show();
			}
		}
	},
...
footer.callback(function(that, i){
	switch(i) {
	     case 1:
	        location.href = T.basepath + 'v1.1/rush.html?f=1';
	        break;
	     case 2:
			location.href = T.basepath + 'v1.1/preferential.html?f=2';
	        break;
		case 3:
			location.href = T.basepath + 'v1.1/activity.html?f=3';
			break;
		//case 4:
			//location.href = T.basepath + 'H5_groupBuying_list.html?footer=4';
			//break;
		case 4:
			location.href = T.basepath + 'my_info.html?f=4';
			break;
	     default:
	        location.href = T.basepath + 'v1.1/rush.html?f=0';
	} 
});
$(function(){
	footer.init();
