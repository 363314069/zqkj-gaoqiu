...
public class CouponsuserServiceImpl extends BaseServiceImpl<CouponsuserDao, CouponsuserEntity> implements CouponsuserService {
    ...
    public Integer addUserCoupons(CouponsuserEntity entity) {
        List<CouponsuserEntity> addList = new ArrayList<>();
        for(int i=0;i< entity.getSum();i++){
            addList.add(entity);
        }
        int nRet = insertList(addList);
        return nRet;
    }
    ...
    public Integer userCouponseExchange(CouponsExchangeBean couponsExchangeBean) {
        int nRet = 0;
        if(couponsExchangeBean.getReduceCouponsSum() != null && couponsExchangeBean.getReduceCouponsSum() > 0){
            String[] delGuids = new String[couponsExchangeBean.getReduceCouponsSum()];
            //先获取要减少的优惠券List
            CouponsuserEntity selectCoupons = new CouponsuserEntity();
            selectCoupons.setCouponsGuid(couponsExchangeBean.getReduceCouponsGuid());
            selectCoupons.setUserGuid(couponsExchangeBean.getUserGuid());
            selectCoupons.setIfUse(0);
            List<CouponsuserEntity> couponsuserList = select(selectCoupons);
            //判断减少数量如果大于数据库总数量直接返回失败
            if(couponsuserList.size() < couponsExchangeBean.getReduceCouponsSum()){
                return 0;
            }
            //拿出要删除的GUID
            for(int i=0;i<couponsExchangeBean.getReduceCouponsSum();i++){
                delGuids[i] = couponsuserList.get(i).getGuid();
            }
            //删除优惠券
            nRet = deleteByGuids(delGuids);
        }

        if(couponsExchangeBean.getAddCouponsSum() != null && couponsExchangeBean.getAddCouponsSum() > 0){
            List<CouponsuserEntity> addList = new ArrayList<>();
            for(int i=0;i<couponsExchangeBean.getAddCouponsSum();i++){
                CouponsuserEntity addCouponsuser = new CouponsuserEntity();
                addCouponsuser.setIfUse(0);
                addCouponsuser.setUserGuid(couponsExchangeBean.getUserGuid());
                addCouponsuser.setCreator(couponsExchangeBean.getUserGuid());
                addCouponsuser.setType(1);
                addCouponsuser.setState(0);
                addCouponsuser.setCouponsGuid(couponsExchangeBean.getAddCouponsGuid());
                addList.add(addCouponsuser);
            }
            nRet += insertList(addList);
        }
        return nRet;
    }
